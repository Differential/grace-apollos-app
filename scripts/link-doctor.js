...
function getClientPackages() {
  const packageJson = JSON.parse(
    fs.readFileSync(`${__dirname}/../GraceChurch/package.json`)
  );
  return [
...
function checkWml() {
  const out = spawnSync('yarn', ['wml', 'list'], {
    encoding: 'utf8',
    cwd: `${__dirname}/../GraceChurch`,
  });
  const { stdout } = out;
  if (stdout.includes('No links set')) {
    return 'No links in GraceChurch have been created. Have you ran `yarn link-packages` ?';
  }
  const linkedPackages = stdout
    .split('\n')
...
function checkEnvVariable() {
  try {
    ...
    const apollosAppsLocationFromEnv = fs
      .readFileSync(`${__dirname}/../GraceChurch/.env`, 'utf8')
    ...
    console.log(
      path.resolve(__dirname, '../GraceChurch/', apollosAppsLocation)
    );
    if (
      !fs.existsSync(
        `${path.resolve(
